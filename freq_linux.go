...
func (dev *FreqDevice) deviceIoControl(bmRequest, request byte, bytes []byte, length int, timeout int) (err error) {
	dev.mutexUSB.Lock()
	switch bmRequest {
	case 0x40:
		_, err = dev.handle.ControlTransfer(0x40, request, 0, 0, bytes, length, timeout) //0x40 out
	case 0xC0:
		_, err = dev.handle.ControlTransfer(0xC0, request, 0, 0, bytes, length, timeout) //0xC0 in
	default:
...
func (dev *FreqDevice) UpdateFreqDataUSB() (err error) {
	...
	err = dev.deviceIoControl(0xC0, 0xB0, freqbytes, len(freqbytes), 1000) //0xC0 in, 0x40 out
...
func (dev *FreqDevice) setLimitWayUSB(wayImpulseCount uint32) (err error) {
	...
	err = dev.deviceIoControl(0x40, 0xB0, freqbytes, len(freqbytes), 1000) //0xC0 in, 0x40 out
